...
class Graph:
    ...
    def visualize_graph(self):
        ...
        node_labels = {node: f"{node}\nCap: {G.nodes[node]['capacity']}\nJobs: {', '.join(self.nodes[node].get_jobs())}" for node in G.nodes}

        nx.draw(G, pos, with_labels=True, labels=node_labels, font_weight='bold')
        #nx.draw_networkx_labels(G, pos, labels=node_labels, font_color='red', font_size=8)  # Add labels with job information
        plt.show()
...
class JobSchedulerGUI:
    def __init__(self, master):
        ...
        # Use ThemedStyle to apply a theme
        self.style = ThemedStyle(master)
        self.style.set_theme("radiance")  # You can choose a different theme here

        self.graph = Graph()

        self.label = Label(master, text="Job Scheduler", style='TLabel')
